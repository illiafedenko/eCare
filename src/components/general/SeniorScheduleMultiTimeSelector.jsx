...
export default function SeniorScheduleMultiTimeSelector({ id, onChange, onCalc, day, date, sID, cgID }) {
  ...
  const getCGAvailabilities = async () => {
    try {
      getAuth().onAuthStateChanged(async (user) => {
        onValue(ref(db, 'cgAvailabilities/' + cgID + '/' + date), (snapshot) => {
          if (snapshot.val() != null)
            setCgAbleTimes(JSON.stringify(snapshot.val()));
        })
      })
    } catch (error) {
      console.log(error.message)
